...
object GameRenderer:
  ...
  def pulse(micros: Long, low: Color, high: Color): Color = {
    ...
    val v1 = ((scala.math.cos(2.0 * math.Pi * t / period) + 1.0)/2.0 * (high._1 - low._1) + low._1)
    val v2 = ((scala.math.cos(2.0 * math.Pi * t / period) + 1.0)/2.0 * (high._2 - low._2) + low._2)
    val v3 = ((scala.math.cos(2.0 * math.Pi * t / period) + 1.0)/2.0 * (high._3 - low._3) + low._3)
  ...
  def colorScheme(micros: Long, phase: GamePhase): ColorScheme =
    phase match {
      case GamePhase.GameOver => ColorScheme(
      ...
      case _ => ColorScheme(
        ...
        gridFill = pulse(micros, (0.65625, 0.0, 0.1679), (0.75625, 0.0, 0.2679)),
      )
    }
  ...
  def render(drawingArea: Ptr[GtkDrawingArea],
    ...
    for {
      ...
      if (state.grid(x, y).exists(_ != GridState.Empty)) {
        ...
        cairo_fill(cr)
        cairo_set_source_rgb(cr, cs.gridOutline._1, cs.gridOutline._2, cs.gridOutline._3)
        cairo_set_line_width(cr, 2.0)
        cairo_rectangle(cr, x * pieceSize, y * pieceSize, pieceSize, pieceSize)
        cairo_stroke(cr)
      } else {
        cairo_set_source_rgb(cr, cs.backgroundGrid._1, cs.backgroundGrid._2, cs.backgroundGrid._3)
